...
namespace DREditor.Dialogues.Editor
{
    ...
    public class DialogueEditor : DialogueEditorBase
    {
        ...
        SerializedProperty diaEventEnum;
        ...
        void CreateForm()
        {
            ...
            {
                if (Resources.Load<DRPlaylist>("Jukebox"))
                {
                    dia.MusicPlaylist = Resources.Load<DRPlaylist>("Jukebox");
                }
                else
                {
                    ...
                    using (new EditorGUILayout.VerticalScope("Box"))
                    {
                        using (new EditorGUILayout.HorizontalScope())
                        {
                            using (new EditorGUILayout.VerticalScope(GUILayout.Width(120)))
                            {
                                ...
                                if (dia.Lines[i].DiaEvents != null)
                                {
                                    for (var j = 0; j < dia.Lines[i].DiaEvents.Count; j++)
                                    {
                                        using (new EditorGUILayout.HorizontalScope())
                                        {
                                            ...
                                            if (GUILayout.Button("x", GUILayout.Width(20)))
                                            {
                                                dia.Lines[i].DiaEvents.Remove(dia.Lines[i].DiaEvents[j]);
                                            }
                                        }
                                    }
                                }
                                ...
                                {
                                    dia.Lines[i].DiaEvents.Add(new DialogueEvent());
                                }
                            ...
                            //Reverted this to its original state since it always return an array null exemption whenever the add line or the copy line button is used. -SeleniumSoul
                            ...
                            using (new EditorGUILayout.VerticalScope())
                            {
                                ...
                                if (GUILayout.Button("*", GUILayout.Width(20)))
                                {
									...
									for (int x = 0; x < dia.Lines[i].DiaEvents.Count; x++)
									{
										Copy.DiaEvents.Add(new DialogueEvent());
										Copy.DiaEvents[x].type = dia.Lines[i].DiaEvents[x].type;
										Copy.DiaEvents[x].PosType = dia.Lines[i].DiaEvents[x].PosType;
										Copy.DiaEvents[x].PanelWindow = dia.Lines[i].DiaEvents[x].PanelWindow;
										Copy.DiaEvents[x].CamPos = dia.Lines[i].DiaEvents[x].CamPos;
									}

                                    // Copy.PanelChange = dia.Lines[i].PanelChange;
                                    // Copy.CamChange = dia.Lines[i].CamChange;
                                    // Copy.MusicChange = dia.Lines[i].MusicChange;
                                    // Copy.Flash = dia.Lines[i].Flash;
                                    // Copy.PosType = dia.Lines[i].PosType;
                                    // Copy.PanelWindow = dia.Lines[i].PanelWindow;
                                    // Copy.CamPos = dia.Lines[i].CamPos;
                            ...
                            if (dia.Lines[i].DiaEvents != null)
                            {
                                using (new EditorGUILayout.VerticalScope())
                                {
                                    for (int d = 0; d < dia.Lines[i].DiaEvents.Count; d++)
                                    {
                                        ...
                                        switch (dia.Lines[i].DiaEvents[d].type)
                                        {
                                            case (DialogueEventType.ChangePanelPositions):
                                                //EditorGUILayout.LabelField("Dialogue Panel");
                                                //dia.Lines[i].DiaEvents[d].PosType = EditorGUILayout.IntPopup(dia.Lines[i].DiaEvents[d].PosType, new string[] { "Panel1Full", "Panel1WithMC", "Panel1WithMC2"}, new int[] { 0, 1, 2});
                                                dia.Lines[i].DiaEvents[d].PosType = EditorGUILayout.DelayedTextField("Position: ", dia.Lines[i].DiaEvents[d].PosType);
                                                EditorGUILayout.HelpBox("Change the positions of the panel windows when talking to somebody.", MessageType.Info, true);
                                                break;

                                            case (DialogueEventType.ChangeCharacterCameraFocus):
                                                dia.Lines[i].DiaEvents[d].PanelWindow = EditorGUILayout.IntPopup("Change Character Camera Focus:", dia.Lines[i].DiaEvents[d].PanelWindow, new string[] { "Middle Panel", "Right Panel", "Left Panel" }, new int[] { 0, 1, 2 });
                                                dia.Lines[i].DiaEvents[d].CamPos = EditorGUILayout.Vector3Field("Camera Focus Position:", dia.Lines[i].DiaEvents[d].CamPos);
                                                EditorGUILayout.HelpBox("Choose which panel camera would change its focused character.\nNote: Changing the Camera Focus Position values to non-zero would force its camera transform.", MessageType.Info, true);
                                                break;

                                            case (DialogueEventType.ChangeMusic):
                                                dia.Lines[i].DiaEvents[d].MusicNum = EditorGUILayout.IntPopup("BGM:", dia.Lines[i].DiaEvents[d].MusicNum, dia.MusicPlaylist.GetAudioTitles(), dia.MusicPlaylist.GetAudioCount());
                                        ...
                                        // if (dia.Lines[i].DiaEvents[d].DiaType is ChangePanelPositions){
                                        //     ChangePanelPositions cpp = new DialogueEvent() as ChangePanelPositions;
                                        //     //dia.Lines[i].DiaEvents[d].DiaType.CPPValue = ChangePanelPositions.EditorDrawer(dia.Lines[i].DiaEvents[d].DiaType.CPPValue);
                                        //     //cpp.EditorDrawer();
                                        //     //cpp.CPPValue.PosType = EditorGUILayout.DelayedTextField("Position: ", cpp.CPPValue.PosType);
                                        //     EditorGUILayout.HelpBox("Change the positions of the panel windows when talking to somebody.", MessageType.Info, true);
                                        //     dia.Lines[i].DiaEvents[d].DiaType = (DialogueEvent)cpp;
                                        // }
